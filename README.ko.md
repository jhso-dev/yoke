...
AI에 기억을 주는 흔한 방식은 관련 구절을 검색해 그대로 붙여넣는 것입니다. yoke는
대신 **레코드**를 주입합니다 — 근거가 붙은 결정, 선호, 사실. 각각이 출처 자료에서
이미 증류된 형태라, 지불하는 토큰이 주변 산문이 아니라 주장 자체를 담습니다.

긴 대화 코퍼스에서 같은 질문, 같은 답변 모델로 측정한 결과: **주입 컨텍스트
~1.5k 토큰 대 구절 검색 ~5.1k 토큰으로 약 3.5배 적으며, 정확도는 측정으로 구분되지
않습니다.** 컨텍스트 창은 에이전트가 가진 가장 희소한 자원이고, 그 여유를 만드는
가장 값싼 방법은 주장이 아닌 텍스트에 토큰을 쓰지 않는 것입니다.

이 형태에는 두 가지가 공짜로 따라옵니다. 주입된 모든 레코드가 인용을 지니는데
붙여넣은 구절은 그럴 수 없고, 주입되는 것은 위의 게이트를 통과한 것이므로 더 작은
컨텍스트가 동시에 통제된 컨텍스트입니다.
...
| **컨텍스트 비용** | 구절이 아니라 레코드: 같은 질문·같은 답변 모델에서 주입 컨텍스트 ~1.5k 토큰 대 구절 검색 ~5.1k 토큰 — 약 3.5배 적고, 정확도는 측정으로 구분되지 않음. |
