...
**토큰당 정답이 청크 검색의 4.6배, 하이브리드 검색기의 18배**입니다 — 후자는 71.4%를
22.8k 토큰 주입으로 삽니다. 작은 모델의 컨텍스트 창 대부분을 질문 하나에 쓰는 셈입니다.
...
| **컨텍스트 비용** | 구절이 아니라 레코드: 같은 질문·같은 답변 모델에서 **1.2k 토큰 대 5.1k**(청크 검색)·**22.8k**(dense+sparse 하이브리드) — 토큰당 정답 4.6배·18배. |
