...
이 하이브리드 행은 벤치마크 공식 평가 조건(프런티어 리더, 전체 문항)에서 84.4%를 받은
시스템이 공개한 파이프라인 그대로이고, 같은 파이프라인이 이 하네스에서는 71.4%입니다.
13포인트는 검색이 아니라 리더의 몫이며, 그만큼을 되돌리면 yoke의 73.8%는 공식 평가
조건에서 ~87%가 됩니다 — 최상위 발표 시스템들과 같은 구간을, 주입 컨텍스트 20분의 1로.
...
| **컨텍스트 비용** | 구절이 아니라 레코드: 같은 질문·같은 답변 모델에서 **1.2k 토큰 대 5.1k**(청크 검색)·**22.8k**(dense+sparse 하이브리드) — 토큰당 정답 5.2배·20배, 정확도는 둘 다보다 높음. |
