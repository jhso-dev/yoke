...
// and most of this suite does real I/O: better-sqlite3 against a temp file, an OpenSearch cluster over
// REST, an MCP server and client on a linked pair — all of it serialized through the single fork above,
// which is not negotiable for the reason given there.
//
...
//
// It also covers the two slowest shapes in the suite without either needing an argument of its own:
// the MCP briefing-cap case (BRIEFING_LIMIT + 4 records through the real gate, each two writes, then a
// bulk verify — over a hundred sqlite transactions before its first assertion) and the live-remote
// adapter suite, where a cold container pays connection setup per case and an index is not queryable
