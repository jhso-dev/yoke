...
- **A briefing has a defined order**, and it is part of this contract: nearest hop first, then
  types the ontology marks `leads` (seeded: decision, term), then most-recently-confirmed first,
  then `id` ascending as the tiebreak. The `leads` rung sits between distance and recency because a
  briefing is what a session sees FIRST: recency alone hands a noisy week's filings the whole page
  while the standing decisions fall off it — measured, 150 unrelated facts left 0 decisions in the
  opening 50. Query paths are untouched; relevance owns their order. Without it `limit` cuts by
