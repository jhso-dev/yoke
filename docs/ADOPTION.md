...
**주간 자가점검**: `node scripts/self-check.mjs --db <스토어> --baseline <파일> --eval --issue` 를
주 1회 걸어두면, 지난주 대비 나빠진 항목만 골라 발견으로 올린다 — 무인 캡처 비율, 결정 전파 지연,
아직 소식일 때 도달한 비율, 결정 1건당 주의력, 재확인 큐, 그리고 주입 품질 게이트. **효율 배수는
목표가 아니다**(docs/RESEARCH.md §7): 배달을 늘리거나 김매기를 멈추면 배수는 오르고 팀은 나빠진다.
점검은 제안만 하고 판단은 사람이 한다.
