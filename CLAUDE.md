...
3. **There are exactly two front adapters: the MCP server and a thin CLI.** We do not build a separate adapter per AI tool (one for Claude, one for Codex, and so on) — they are all MCP clients. The CLI is a **client**: every command that touches the corpus goes to a `yoke serve` (loopback by default), so an operation is implemented once, behind the routes. Only the commands that act on a machine — `init`, `serve`, `ui`, `mcp`, `token`, `backup`, `restore`, `export` — open a store. A command that reads the OUTSIDE world (the `connect` connectors, `relate`) runs that half locally, where its credentials, files and model are, and hands what it found to the same gate over HTTP.
