...
2. **Every storage backend implements the same storage port interface and passes a shared conformance test suite.** Backend-specific behavior must never leak into the core — and no backend gets a user-facing capability the others lack. sqlite is one backend among several, not the privileged one: a command that only works on a single local file (backup, point-in-time export) does not belong in the product, because it makes what yoke can do depend on which database is underneath. Optimizations may differ per backend (`listHistory`, `rebuildFts`) as long as every caller still gets the same answer without them.
3. **There are exactly two front adapters: the MCP server and a thin CLI.** We do not build a separate adapter per AI tool (one for Claude, one for Codex, and so on) — they are all MCP clients. The CLI is a **client**: every command that touches the corpus goes to a `yoke serve` (loopback by default), so an operation is implemented once, behind the routes. Only the commands that act on a machine — `init`, `serve`, `ui`, `mcp`, `token` — open a store. A command that reads the OUTSIDE world (the `connect` connectors, `relate`) runs that half locally, where its credentials, files and model are, and hands what it found to the same gate over HTTP.
