...
step 2's local store. That one variable is the whole configuration: the hooks and the agent both
reach the server through the CLI, which exchanges the developer's `gh` login for a yoke credential
on first use and refreshes it from then on (the server must set `YOKE_GITHUB_ORG`). Nothing to
paste, and no database password on anybody's laptop. Check the preconditions out loud here:
`gh auth token` must print a token, and
