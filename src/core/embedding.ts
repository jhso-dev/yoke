...
export function serializeText(
  ...
  if (proseKeyEnabled()) return proseText(type, attributesJson, ontology ?? []);
 ...
 * adapters pass env in (`envKeywordWeight`). It is here because the other half of this key is
 * written by three storage adapters that take a path, not an env, so passing it properly means a new
 * field on the storage port for an A/B gate. Thread it through the port if prose outlives the
 * experiment; delete this function if it does not.
 *
