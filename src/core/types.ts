...
export interface Provenance {
   ...
   * time, which is not the knowledge's event time. Written only by lifecycle.transition; absent on
   * every commit-written row, and on lifecycle rows written before the two times were separated
   * (those carry the transition instant in `occurred_at`, which is the defect this field fixes). */
