...
export async function backfillEmbeddings(
  ...
  // cannot apply is not an error, and the caller reports the zero. Reached in practice by a managed
  // Postgres without pgvector — that adapter assigns `putEmbedding` in `init()` only when the
  // extension is there, so the absence is real rather than hypothetical.
