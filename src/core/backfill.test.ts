...
import { backfillEmbeddings, backfillOccurredAt } from "./backfill.js";
...
describe("backfillOccurredAt", () => {
  ...
  it("restores the event time from history and reports old -> new", async () => {
    ...
    expect(changes).toEqual([{ id, from: verifiedAt, to: said }]);

    const cur = await port.getEntity(id);
    expect(cur?.provenance.occurred_at).toBe(said);
