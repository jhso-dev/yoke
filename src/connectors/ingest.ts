...
/**
 * ONE source item through the commit gate: probe by external id, compare, merge, commit. Every bulk
 * path routes each of its items through here — a second copy of this sequence is a second set of the
 * things it has to get right (the merge below, the critical section, the probe's `terms`).
 *
 * `prov` is the actor and origin the source is recorded under; the instants are this function's, since
 * which clock a source item is stamped with is part of what the gate guarantees.
 *
 * Throws `CommitRejected` for an item that cannot be recorded — per item, so the caller's loop counts
 * it and keeps going rather than abandoning the rest of the run.
 */
export async function ingestItem(
  ...
  const ns = opts?.ns;
  let result: ItemOutcome | undefined;
  ...
  const handleItem = async (): Promise<void> => {
    ...
    if (stored && sameContent(stored, pulled)) {
      result = { outcome: "skipped", id: stored.id };
      return;
    }
    ...
    // edited to drop a column, an rdb mapping's `columns` edited to drop one, an `idColumn`/
    // `occurredAtColumn` retargeted), the merge keeps the dropped fields on the head version rather
    // than removing them — data must not vanish from an append-only path because of a config edit. A
    // connector that dropped a field is indistinguishable from one that never had it, and silently
    // deleting stored knowledge on that guess is the more expensive mistake. `sameContent` compares
    // only the pulled keys for the same reason: the connector's authority ends at the fields it produces.
    ...
    const { entity } = await commit(
      ...
      { ...prov, occurred_at: occurredAt ?? now },
  ...
  // A SourceItem is an EntityInput, so the section either set this or threw.
  return result as ItemOutcome;
}
...
export async function ingest(
  ...
  for await (const item of connector.pull(since)) {
    ...
    try {
      ...
      rejected.push(`${item.externalId}: ${e.message}`);
    }
  }
  return {
    added,
