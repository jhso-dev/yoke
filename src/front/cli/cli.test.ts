...
describe("backfill --embeddings", () => {
  ...
  // The whole loop the index-key meta exists for. `YOKE_INDEX_KEY` chooses only here, on --rebuild;
  // every other command reads the choice back off the store, so a command run without the flag can no
  // longer re-key the rows it touches and leave half the index in each representation.
  it("--rebuild records the index key, and later writes keep it without the flag", async () => {
    const db = newDb();
    const ftsOf = (id: string): string => {
      const d = new Database(db, { readonly: true });
      try {
        return (
          d.prepare("SELECT text FROM entities_fts WHERE id = ?").get(id) as {
            text: string;
          }
        ).text;
      } finally {
    ...
    const lastId = (): string =>
      (JSON.parse(logs.at(-1) as string) as { id: string }).id;

    expect(await runCli(["init", "--db", db], {})).toBe(0);
    expect(
      await runCli(
        ["add", "fact", "--db", db, "--json", "--attr", "statement=blogs"],
        {},
      ),
    ).toBe(0);
    const first = lastId();
    expect(ftsOf(first)).toContain('{"statement"');

    // The flag now set, and it changes NOTHING on its own: this store has already indexed a record,
    // so its recorded variant decides. (Stubbed as well as passed: `add` reads the CLI's env, the
    // stamp inside core reads the ambient one — the whole point is that neither can re-key a store.)
    vi.stubEnv("YOKE_INDEX_KEY", "prose");
    expect(
      await runCli(
        ["add", "fact", "--db", db, "--json", "--attr", "statement=second"],
        { YOKE_INDEX_KEY: "prose" },
      ),
    ).toBe(0);
    expect(ftsOf(lastId())).toContain('{"statement"');

    // The one command that may change it. No embedder configured, so only the keyword half rebuilds —
    // which is the half this test can see.
    expect(
      await runCli(["backfill", "--embeddings", "--rebuild", "--db", db], {
        YOKE_INDEX_KEY: "prose",
      }),
    ).toBe(0);
    vi.unstubAllEnvs();
    expect(logs.join("\n")).toContain("rebuilt the keyword index");
    expect(ftsOf(first)).toBe("fact. blogs");

    // A later command with NO flag — the shape of the defect (`backfill --occurred-at` rewriting rows
    // from a shell that never exported it).
    expect(
      await runCli(
        [
          "add",
          ...
          "--json",
          "--attr",
          "statement=still prose",
        ],
        {},
      ),
    ).toBe(0);
    expect(ftsOf(lastId())).toBe("fact. still prose");
    expect(ftsOf(first)).toBe("fact. blogs");
  });
});
